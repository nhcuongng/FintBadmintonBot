...
app.post('/update-sheet-id', (req, res) => {
  try {
    const { sheetId, threadId } = req.body
...
app.get('/cron-list', (req, res) => {
  try {
    ...
    Object.keys(gateway.subject).forEach((key) => {
      ...
      const {
        cronExpression,
        badmintonFieldName: fieldNumber,
        paramsBot,
        chatTitle
      } = pollController
...
app.get('/send-poll', async (req, res) => {
  try {
    const promises = Object.keys(gateway.subject).map(async (key) => {
      ...
      const messageId = await callApiTelegramCreatePoll(
      ...
      if (messageId) {
        ...
        await autoPinnedTheNewOne(
          messageId,
          previousPinnedId,
          pollController.paramsBot.chat_id
        )
